...
RANK=6                         # rank 6 ≈ l1_alpha=0.0027 (paper default for Pythia-70M)
DICT_RATIOS="4 8"
N_CHUNKS=16
N_EPOCHS=5
BATCH_SIZE=1024
...
echo "========================================================"
echo "STEP 1: Collect activations and train SAEs"
echo "========================================================"
...
for RATIO in $DICT_RATIOS; do
    ...
    python activation_patching.py \
        ...
        --out            "${RESULTS_DIR}/patch_hist_r${RATIO}.png"
    ...
    python patch_curve.py \
        ...
        --acts_nocot_dir "$NOCOT_ACTS" \
        --max_samples    "$MAX_SAMPLES" \
        --out            "${RESULTS_DIR}/patch_curve_r${RATIO}.png" \
        --save_stats     "${RESULTS_DIR}/ttest_r${RATIO}.txt"
done
